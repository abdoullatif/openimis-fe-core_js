...
import { MUTATION_LOG_TASKBAR_PROJECTION, shouldStopMutationPolling } from "./helpers/mutationTaskBar";
...
const CUSTOM_FILTER_VALUE_SUGGESTIONS_PROJECTION = () => ["value", "label"];

export function fetchCustomFilterValueSuggestions({
  moduleName,
  objectTypeName,
  field,
  search,
  ...
  const params = [
    `moduleName: "${formatGQLString(moduleName)}"`,
    `objectTypeName: "${formatGQLString(objectTypeName)}"`,
    `field: "${formatGQLString(field)}"`,
    `search: "${formatGQLString(search.trim())}"`,
    `limit: ${limit}`,
  ];
  if (uuidOfObject) {
    params.push(`uuidOfObject: "${formatGQLString(uuidOfObject)}"`);
  }
  const payload = formatQuery(
    "customFilterValueSuggestions",
    params,
    CUSTOM_FILTER_VALUE_SUGGESTIONS_PROJECTION(),
  );
  return graphql(payload, "FETCH_CUSTOM_FILTER_VALUE_SUGGESTIONS");
}
...
export function waitForMutation(clientMutationId) {
  return async (dispatch) => {
    ...
    do {
      ...
      const response = await dispatch(
        ...
        query ($clientMutationId: String) {
          mutationLogs (clientMutationId: $clientMutationId, orderBy: ["-requestDateTime"], first: 1) {
            edges {
              node {
                status
                clientMutationId
    ...
    } while (attempts++ < 10 && (!res || !shouldStopMutationPolling(res)));
